...
import 'package:allenrealestateflutter/core/services/real_estate_service/implementations/mock/parsers.dart'
    as parser;
...
class RealEstateMockService extends RealEstateService {
  ...
  Future<AsyncResult<List<RealEstateListItem>>> getPopularRealEstates(
    ...
    log('Fetching results. page: $page, size: $size',
        name: '$tag/getPopularRealEstates');
  ...
  Future<AsyncResult<List<RealEstateListItem>>> getRealEstatesByQuery(
    ...
    log('Fetching results. page: $page, size: $size, query: $query',
        name: '$tag/getRealEstatesByQuery');
    List<RealEstateListItem> results = mockReList
  ...
  Future<AsyncResult<RealEstate>> getRealEstateById({String id}) async {
    ...
    List<RealEstate> data =
        mockReList.map((e) => parser.realEstateFromJson(e)).toList();
    RealEstate results =
        data.firstWhere((RealEstate element) => element.id == id, orElse: null);
    if (results == null) {
  ...
  Future<AsyncResult<List<RealEstateListItem>>> getSimilarRealEstatesById(
      {String id, int page = 1, int size = 10}) async {
    log('Fetching results id: $id. page: $page, size: $size',
        name: '$tag/getSimilarRealEstatesById');
    List<RealEstateListItem> results = mockReList
        .sublist((page - 1) * size, (page - 1) * size + size)
        .map((e) => parser.realEstateListItemFromJson(e))
        .toList();
  ...
  Future<AsyncResult<List<RealEstateListItem>>> getRealEstatesByCategoryId(
      {String id, int page = 1, int size = 10}) async {
    log('Fetching results id: $id. page: $page, size: $size',
        name: '$tag/getRealEstatesByCategoryId');
    List<RealEstateListItem> results = mockReList
        .sublist((page - 1) * size, (page - 1) * size + size)
        .map((e) => parser.realEstateListItemFromJson(e))
        .toList();
